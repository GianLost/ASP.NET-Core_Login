// Arquivo responsável por conter todas as declarações de elementos para utilização das funções de animação;

// Elementos para a página de login;
export const loginPasswordInput = $("#login-password");
export const loginTogglePasswordButton = $("#toggle-password");
export const loginPasswordIcon = $("#password-icon");
export const iconLogin = $("#login-btn");

// Elementos para a página Home
export const iconLogout = $("#close-link");

// Elementos para a página de registro de usuários
export const userPasswordInput = $("#user-Password");
export const userTogglePasswordButton = $("#user-toggle-password");
export const userPasswordIcon = $("#user-password-icon");

export const userConfPasswordInput = $("#user-confPass");
export const userConfTogglePasswordButton = $("#user-toggle-confpassword");
export const userConfPasswordIcon = $("#user-confpassword-icon");